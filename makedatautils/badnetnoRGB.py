...
if __name__ == '__main__':
    ...
    cfg_path = 'config/makedata/tri/placer1.yaml'
    # 解析配置文件
    with open(cfg_path, 'r') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)
        config = edict(config)
    badnet = BadnetnoRGB(cfg=config,debug=True)
    badnet.puttrigger('data/tri/val_place1/藏AL6010_0_badnet_badnet.png', 'data/test_detect')
